...
$stagingDir = "$currentDir\build_staging"
$installerProjectDir = "$stagingDir\installer_project"
$installerPath = "$currentDir\VibeAlarmInstaller.exe"
$sedPath = "$currentDir\VibeAlarm.sed"
...
dotnet publish "$currentDir\VibeAlarm.csproj" -c Release -r win-x64 --self-contained true -p:PublishSingleFile=true -p:PublishReadyToRun=true -o "$stagingDir\publish"
...
Copy-Item -Path "$currentDir\Assets\alarm.wav" -Destination "$stagingDir\alarm.wav"
...
$sedNote = @"
This project now builds VibeAlarmInstaller.exe with a generated self-contained .NET installer.
IExpress was intentionally bypassed because it returned success without producing an EXE on this machine.
"@
$sedNote | Out-File -FilePath $sedPath -Encoding ascii
