...
namespace VibeAlarm.UI.Theming
{
    ...
    public static class FontRegistry
    {
        ...
        /// <summary>Embedded resource names, and the family name each one registers as.
        /// NOTE: the SemiBold static face's GDI+ family name is "Inter Semi Bold" (with a
        /// space) — the candidates below must match it exactly or the lookup silently
        /// falls through to plain Inter.</summary>
        private static readonly (string Resource, string Family)[] EmbeddedFaces =
        {
            ("VibeAlarm.Assets.Fonts.Inter-Regular.ttf",         "Inter"),
            ("VibeAlarm.Assets.Fonts.Inter-SemiBold.ttf",        "Inter Semi Bold"),
            ("VibeAlarm.Assets.Fonts.JetBrainsMono-Regular.ttf", "JetBrains Mono"),
        };

        /// <summary>Preferred family per role, then Windows-native fallbacks in descending order of fit.
        /// The fallback chain IS the shipped look whenever the embedded Inter faces are absent, so
        /// it must stay on-voice: Segoe UI Variable (the closest system look-alike) before the older
        /// Segoe faces.</summary>
        private static readonly string[] DisplayCandidates = { "Inter Semi Bold", "Inter", "Segoe UI Variable Display", "Bahnschrift", "Segoe UI" };
